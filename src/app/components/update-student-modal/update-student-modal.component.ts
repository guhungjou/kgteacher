...
export class UpdateStudentModalComponent implements OnInit, OnChanges {
  ...
  onOk() {
    ...
    this.updateStudent(this.data.id, name, gender, remark, device, this.classID);
  }

  async updateStudent(id: number, name: string, gender: string, remark: string, device: string, classID: number) {
    try {
      this.loading = true;
      const r = await this.api.updateStudent(id, name, gender, remark, device, classID);
      if (r.status === 21001) {
